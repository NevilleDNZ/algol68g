...
/* Posix copies arguments from the command line. */
  SCAN_ERROR (argc <= 1, NULL, "no input file specified (specify -help for help)");
...
/* We translate the program. */
  if (a68_prog.files.generic_name == NULL || strlen (a68_prog.files.generic_name) == 0) {
    SCAN_ERROR (!a68_prog.options.version, NULL, "no input file specified (specify -help for help)");
  } else {
    get_stack_size ();
...
/* File set-up. */
  SCAN_ERROR (a68_prog.files.generic_name == NULL, NULL, "no input file specified (specify -help for help)");
...
*/
  ...
  whether_extension (".ALGOL68G");
  if (a68_prog.files.source.fd == -1) {
    sprintf (edit_line, "cannot open source file `%s'", a68_prog.files.source.name);
...
/* Write listing. */
  if (a68_prog.files.listing.opened) {
    ...
    sprintf (output_line, "%s%s %s", BARS, A68G_NAME, REVISION);
    io_write_string (a68_prog.files.listing.fd, output_line);
    sprintf (output_line, "\n%sSource listing for \"%s\"", BARS, a68_prog.files.source.name);
    io_write_string (a68_prog.files.listing.fd, output_line);
...
{
  io_close_tty_line ();
  sprintf (output_line, "%s: %s, %s", A68G_NAME, REVISION, RELEASE_DATE);
  io_write_string (STDOUT_FILENO, output_line);
#ifdef __GNUC__
#ifdef __VERSION__
  sprintf (output_line, "\n%s: compiled on %s %s with gcc %s", A68G_NAME, __DATE__, __TIME__, __VERSION__);
#else
  sprintf (output_line, "\n%s: compiled on %s %s with gcc %d.%d", A68G_NAME, __DATE__, __TIME__, __GNUC__, __GNUC_MINOR__);
#endif
#else
  sprintf (output_line, "\n%s: compiled on %s %s", A68G_NAME, __DATE__, __TIME__);
#endif
  io_write_string (STDOUT_FILENO, output_line);
#ifdef HAVE_PLOTUTILS
  ...
  io_write_string (STDOUT_FILENO, output_line);
#endif
  sprintf (output_line, "\n%s: alignment: %d bytes", A68G_NAME, ALIGNMENT);
  io_write_string (STDOUT_FILENO, output_line);
  default_mem_sizes ();
  sprintf (output_line, "\n%s: default frame stack size: %ld kB", A68G_NAME, frame_stack_size / KILOBYTE);
  io_write_string (STDOUT_FILENO, output_line);
  sprintf (output_line, "\n%s: default expression stack size: %ld kB", A68G_NAME, expr_stack_size / KILOBYTE);
  io_write_string (STDOUT_FILENO, output_line);
  sprintf (output_line, "\n%s: default heap size: %ld kB", A68G_NAME, heap_size / KILOBYTE);
  io_write_string (STDOUT_FILENO, output_line);
  sprintf (output_line, "\n%s: default handle pool size: %ld kB", A68G_NAME, handle_pool_size / KILOBYTE);
  io_write_string (STDOUT_FILENO, output_line);
  sprintf (output_line, "\n%s: default stack overhead: %ld kB", A68G_NAME, storage_overhead / KILOBYTE);
  io_write_string (STDOUT_FILENO, output_line);
}
