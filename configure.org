...
# Process current option
	...
	-O4)
		opt=`echo $opt "-O4"` ;;
	-O5)
		opt=`echo $opt "-O4 -funroll-loops -march=prescott -mno-align-double -fmove-all-movables -maccumulate-outgoing-args -m32"` ;;
	-Os)
		opt=`echo $opt -Os` ;;
	-threads | --threads)
		threads=;;
...
then
	...
	do
		echo $f
		sed "/<!-- Header -->/r header" < $f | sed "/<!-- Footer -->/r footer" | sed "/<!-- Navigation bar -->/r navigation" | sed "s/REVISION/$REVISION/g" | sed "s/TIME_STAMP/$TIME_STAMP/g" | sed "s/PACKAGE/$PACKAGE/g" | sed "s/LINK_COLOUR/$LINK_COLOUR/g" | sed "s/BACKGROUND_COLOUR/$BACKGROUND_COLOUR/g" > ../$f
	done
