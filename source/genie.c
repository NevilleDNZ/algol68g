...
@param compile_lib Handle to compiled library.
**/
...
static PROP_T genie_widening_int_to_real (NODE_T * p)
{
  ...
  EXECUTE_UNIT (SUB (p));
  INCREMENT_STACK_POINTER (p, SIZE_AL (A68_REAL) - SIZE (MODE (INT)));
...
/* Miscellaneous widenings */
    ...
    z = heap_generator (p, MODE (ROW_BOOL), SIZE_AL (A68_ARRAY) + SIZE_AL (A68_TUPLE));
    row = heap_generator (p, MODE (ROW_BOOL), BITS_WIDTH * SIZE (MODE (BOOL)));
    DIM (&arr) = 1;
    MOID (&arr) = MODE (BOOL);
    ELEM_SIZE (&arr) = SIZE (MODE (BOOL));
    SLICE_OFFSET (&arr) = 0;
    FIELD_OFFSET (&arr) = 0;
    ARRAY (&arr) = row;
    LWB (&tup) = 1;
    ...
    base = ADDRESS (&row) + SIZE (MODE (BOOL)) * (BITS_WIDTH - 1);
    bit = 1;
    for (k = BITS_WIDTH - 1; k >= 0; k--, base -= SIZE (MODE (BOOL)), bit <<= 1) {
...
/* Make [] BOOL */
    z = heap_generator (p, MODE (ROW_BOOL), SIZE_AL (A68_ARRAY) + SIZE_AL (A68_TUPLE));
    row = heap_generator (p, MODE (ROW_BOOL), width * SIZE (MODE (BOOL)));
    DIM (&arr) = 1;
    MOID (&arr) = MODE (BOOL);
    ELEM_SIZE (&arr) = SIZE (MODE (BOOL));
    SLICE_OFFSET (&arr) = 0;
    ...
    K (&tup) = 0;
    PUT_DESCRIPTOR (arr, tup, &z);
    base = ADDRESS (&row) + (width - 1) * SIZE (MODE (BOOL));
    k = width;
    while (k > 0) {
      unsigned bit = 0x1;
      int j;
...
/* Move argument from stack to locale */
      ...
      VALUE (&w) = A68_TRUE;
      *(A68_BOOL *) & u[0] = w;
      COPY (&(u[SIZE (MODE (BOOL))]), v, SIZE (MOID (t)));
      u = &(u[SIZE (MODE (BOOL)) + SIZE (MOID (s))]);
      v = &(v[SIZE (MOID (t))]);
      FORWARD (s);
    }
...
@param p Node in syntax tree.
@param m Mode of procedure.
@param proc Procedure to call.
@param pop_sp Stack pointer at call.
@param pop_fp Frame pointer at call.
@return A propagator for this action.
**/
...
@param p Node in syntax tree.
@param tup Pointer to TUPLE variable.
...
/* Trimming selects a subarray from an array */
    ...
    ref_desc_copy = heap_generator (p, MOID (p), SIZE_AL (A68_ARRAY) + DIM (DEFLEX (result_mode)) * SIZE_AL (A68_TUPLE));
/* Get descriptor */
    POP_REF (p, &z);
/* Get indexer */
    CHECK_REF (p, z, MOID (SUB (p)));
    ...
    ref_old = ADDRESS (&z) + SIZE_AL (A68_ARRAY);
    ref_new = ADDRESS (&ref_desc_copy) + SIZE_AL (A68_ARRAY);
...
/* INT denotation */
    ...
    UNIT (&self) = genie_constant;
    STATUS (&z) = INIT_MASK;
    CONSTANT (GINFO (p)) = (void *) get_heap_space ((size_t) SIZE (MODE (INT)));
    SIZE (GINFO (p)) = SIZE (MODE (INT));
    COPY (CONSTANT (GINFO (p)), &z, SIZE (MODE (INT)));
...
/* REAL denotation */
    ...
    STATUS (&z) = INIT_MASK;
    UNIT (&self) = genie_constant;
    CONSTANT (GINFO (p)) = (void *) get_heap_space ((size_t) SIZE_AL (A68_REAL));
    SIZE (GINFO (p)) = SIZE_AL (A68_REAL);
    COPY (CONSTANT (GINFO (p)), &z, SIZE_AL (A68_REAL));
...
/* BITS denotation */
    ...
    UNIT (&self) = genie_constant;
    STATUS (&z) = INIT_MASK;
    CONSTANT (GINFO (p)) = (void *) get_heap_space ((size_t) SIZE_AL (A68_BITS));
    SIZE (GINFO (p)) = SIZE_AL (A68_BITS);
    COPY (CONSTANT (GINFO (p)), &z, SIZE_AL (A68_BITS));
...
/* Multiple selections */
  if (selection_of_name && (IS (SUB (struct_mode), FLEX_SYMBOL) || IS (SUB (struct_mode), ROW_SYMBOL))) {
    ...
    desc_size = SIZE_AL (A68_ARRAY) + dims * SIZE_AL (A68_TUPLE);
...
@param p Node in syntax tree.
@param pop_sp Value to restore stack pointer.
@param seq Chain to link nodes into.
**/
...
A68_REF c_string_to_row_char (NODE_T * p, char *str, int width)
{
  ...
  DIM (&arr) = 1;
  MOID (&arr) = MODE (CHAR);
  ELEM_SIZE (&arr) = SIZE_AL (A68_CHAR);
  SLICE_OFFSET (&arr) = 0;
  FIELD_OFFSET (&arr) = 0;
  ARRAY (&arr) = row;
  LWB (&tup) = 1;
  ...
  base = ADDRESS (&row);
  for (k = 0; k < width; k++) {
    A68_CHAR *ch = (A68_CHAR *) & (base[k * SIZE_AL (A68_CHAR)]);
    STATUS (ch) = INIT_MASK;
...
@param p Node in syntax tree.
@param str String to convert.
@param width Width of string (if different from strlen (str)).
@return A68 string
**/
...
A68_REF empty_row (NODE_T * p, MOID_T * u)
{
  ...
  v = SUB (u);
  dim = DIM (u);
  dsc = heap_generator (p, u, SIZE_AL (A68_ARRAY) + dim * SIZE_AL (A68_TUPLE));
...
/* Make the new descriptor */
  nrow = heap_generator (p, rmod, SIZE_AL (A68_ARRAY) + DIM (nmod) * SIZE_AL (A68_TUPLE));
...
A68_REF genie_make_row (NODE_T * p, MOID_T * elem_mode, int len, ADDR_T sp)
{
  ...
  A68_TUPLE *tup;
  int k;
  new_row = heap_generator (p, MOID (p), SIZE_AL (A68_ARRAY) + SIZE_AL (A68_TUPLE));
  new_arr = heap_generator (p, MOID (p), len * SIZE (elem_mode));
  GET_DESCRIPTOR (arr, tup, &new_row);
  DIM (arr) = 1;
  MOID (arr) = elem_mode;
  ELEM_SIZE (arr) = SIZE (elem_mode);
  SLICE_OFFSET (arr) = 0;
  FIELD_OFFSET (arr) = 0;
  ARRAY (arr) = new_arr;
  LWB (tup) = 1;
...
/* ROWING NIL yields NIL */
  ...
  new_row = heap_generator (p, SUB (dst_mode), SIZE_AL (A68_ARRAY) + SIZE_AL (A68_TUPLE));
...
/* Make new descriptor */
  new_row = heap_generator (p, dst_mode, SIZE_AL (A68_ARRAY) + DIM (SUB (dst_mode)) * SIZE_AL (A68_TUPLE));
  name = heap_generator (p, dst_mode, A68_REF_SIZE);
...
@param tmp Used for bound checks, NIL if irrelevant.
...
/* REF SOUND */
    ...
    COPY ((BYTE_T *) w, ADDRESS (old), SIZE (MODE (SOUND)));
    owd = ADDRESS (&(DATA (w)));
...
/* Make new array */
    ...
    nrow = heap_generator (p, m, SIZE_AL (A68_ARRAY) + DIM (oarr) * SIZE_AL (A68_TUPLE));
...
static PROP_T genie_diagonal_function (NODE_T * p)
{
  ...
  new_row = heap_generator (p, m, SIZE_AL (A68_ARRAY) + SIZE_AL (A68_TUPLE));
  DIM (&new_arr) = 1;
  MOID (&new_arr) = m;
...
static PROP_T genie_transpose_function (NODE_T * p)
{
  ...
  POP_OBJECT (p, &row, A68_ROW);
  GET_DESCRIPTOR2 (arr, tup1, tup2, &row);
  new_row = heap_generator (p, m, SIZE_AL (A68_ARRAY) + 2 * SIZE_AL (A68_TUPLE));
  new_arr = *arr;
  new_tup1 = *tup2;
  new_tup2 = *tup1;
...
/* Construct an array of sorted strings */
    ...
    row = heap_generator (p, MODE (ROW_STRING), size * SIZE (MODE (STRING)));
    DIM (&arrn) = 1;
    MOID (&arrn) = MODE (STRING);
    ELEM_SIZE (&arrn) = SIZE (MODE (STRING));
    SLICE_OFFSET (&arrn) = 0;
    FIELD_OFFSET (&arrn) = 0;
    ARRAY (&arrn) = row;
    LWB (&tupn) = 1;
    ...
    if (INITIALISED (z) && LOCALE (z) != NO_HANDLE && !(STATUS_TEST (LOCALE (z), COOKIE_MASK))) {
      ...
      for (; s != NO_PACK; FORWARD (s)) {
        if (VALUE ((A68_BOOL *) & u[0]) == A68_TRUE) {
          colour_object (&u[SIZE (MODE (BOOL))], MOID (s));
        }
        u = &(u[SIZE (MODE (BOOL)) + SIZE (MOID (s))]);
      }
...
@param p Position in the syntax tree.
@param faddr Field address in STRUCT.
@param decl Declarer in the syntax tree.
@param cur_sp Current stack pointer.
@param top_sp Stack pointer value to restore.
**/
...
@param p Decl in the syntax tree.
@param addr Field address in STRUCT.
@param decl Declarer in the syntax tree.
@param cur_sp Current stack pointer.
**/
...
void genie_level_sema_int (NODE_T * p)
{
  ...
  A68_REF s;
  POP_OBJECT (p, &k, A68_INT);
  s = heap_generator (p, MODE (INT), SIZE (MODE (INT)));
  * DEREF (A68_INT, &s) = k;
  PUSH_REF (p, s);
}
