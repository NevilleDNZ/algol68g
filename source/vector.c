...
#define TEST_LENGTH(a, b) {\
    if ((a) != (b)) {\
      diagnostic (A_RUNTIME_ERROR, p, "M vectors must have equal length", MODE (ROW_REAL));\
...
/*-------1---------2---------3---------4---------5---------6---------7---------+
PROC ([] REAL, REAL) VOID vector set                                          */

void
genie_vector_set (NODE_T * p)
{
  A68_REF a;
  A68_REAL y;
  A68_ARRAY *arr_a;
...
/*-------1---------2---------3---------4---------5---------6---------7---------+
PROC (REF [] REAL, [] REAL, REAL) VOID vector times scalar                    */

void
genie_vector_times_scalar (NODE_T * p)
{
  A68_REF a, b;
  A68_REAL z;
...
/*-------1---------2---------3---------4---------5---------6---------7---------+
PROC (REF [] REAL, [] REAL) VOID vector move                                  */

void
genie_vector_move (NODE_T * p)
{
...
/*-------1---------2---------3---------4---------5---------6---------7---------+
PROC (REF [] REAL, [] REAL, [] REAL) VOID vector add                          */

void
genie_vector_add (NODE_T * p)
{
  A68_REF a, b, c;
...
/*-------1---------2---------3---------4---------5---------6---------7---------+
PROC (REF [] REAL, [] REAL, [] REAL) VOID vector sub                          */

void
genie_vector_sub (NODE_T * p)
{
  A68_REF a, b, c;
  A68_ARRAY *arr_a, *arr_b, *arr_c;
  A68_TUPLE *tup_a, *tup_b, *tup_c;
...
/*-------1---------2---------3---------4---------5---------6---------7---------+
PROC (REF [] REAL, [] REAL, [] REAL) VOID vector mul                          */

void
genie_vector_mul (NODE_T * p)
{
  A68_REF a, b, c;
  A68_ARRAY *arr_a, *arr_b, *arr_c;
  A68_TUPLE *tup_a, *tup_b, *tup_c;
...
/*-------1---------2---------3---------4---------5---------6---------7---------+
PROC (REF [] REAL, [] REAL, [] REAL) VOID vector div                          */

void
genie_vector_div (NODE_T * p)
{
  A68_REF a, b, c;
  A68_ARRAY *arr_a, *arr_b, *arr_c;
  A68_TUPLE *tup_a, *tup_b, *tup_c;
...
/*-------1---------2---------3---------4---------5---------6---------7---------+
PROC ([] REAL, [] REAL) REAL vector inproduct                                 */

void
genie_vector_inner_product (NODE_T * p)
{
  A68_REF a, b;
...
/* Optimised loop for vector element access */
  ...
  dsum = stack_mp (p, digits);
  SET_MP_ZERO (dsum, digits);
  dfac = stack_mp (p, digits);
  for (k = 0; k < l_a; k++)
    {
      A68_REAL *y = (A68_REAL *) & (b_b[j]);
      A68_REAL *x = (A68_REAL *) & (b_a[i]);
